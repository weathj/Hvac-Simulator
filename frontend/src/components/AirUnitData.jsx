import { useState, useRef, useEffect } from 'react'
import { Panel, StatRow } from './PanelComponents'
import { useAHU } from '../hooks/useAHU'
import { useForce } from '../hooks/useForce'
import './PanelComponents.css'

export default function AirUnitData() {
    const { data, error } = useAHU()
    const ahu = data?.[0]

    if (error) return <div className="fault-banner">&#x26a0; FAULT &mdash; {error}</div>

    return (
        <div className="panel-grid">
                <Panel title="Supply Air" variant="supply">
                    <StatRow label="Temperature"  value={ahu?.sa_temp}           unit=" °F"     decimals={2} />
                    <StatRow label="Humidity"     value={ahu?.sa_humidity}       unit=" %"                   />
                    <StatRow label="BTU"          value={ahu?.sa_btu}            unit=" BTU/hr"              />
                    <StatRow label="Flow"         value={ahu?.sa_flow}           unit=" CFM"                 />
                    <StatRow label="Fan Speed"    value={ahu?.sa_fan_speed}      unit=" %"                   />
                    <StatRow label="Cooling Coil" value={ahu?.cooling_coil_temp} unit=" °F"     decimals={2} />
                    <StatRow label="Heating Coil" value={ahu?.heating_coil_temp} unit=" °F"     decimals={2} />
                </Panel>

                <Panel title="Mixed Air" variant="mixed">
                    <StatRow label="Temperature"     value={ahu?.ma_temp}             unit=" °F"     decimals={2} />
                    <StatRow label="Humidity"        value={ahu?.ma_humidity}         unit=" %"                   />
                    <StatRow label="BTU"             value={ahu?.ma_btu}              unit=" BTU/hr"              />
                    <StatRow label="Damper Position" value={ahu?.ma_damper_position}  unit="%"                    />
                </Panel>

                <Panel title="Return Air" variant="return">
                    <StatRow label="Temperature" value={ahu?.ra_temp} unit=" °F"  decimals={2} />
                    <StatRow label="Flow"        value={ahu?.ra_flow} unit=" CFM"              />
                    <StatRow label="Fan Speed"   value={ahu?.ra_fan_speed} unit=" %"           />
                </Panel>
                
                <Panel title="Exhaust Air" variant="exhaust">
                    <StatRow label="Damper Position" value={ahu?.ea_damper_position} unit="%" />
                </Panel>
                <Panel title="Outside Air" variant="outside">
                    <StatRow label="Temperature"     value={ahu?.oa_temp}           unit=" °F"     decimals={2} />
                    <StatRow label="Humidity"        value={ahu?.oa_humidity}       unit=" %"                   />
                    <StatRow label="BTU"             value={ahu?.oa_btu}            unit=" BTU/hr"              />
                    <StatRow label="Outdoor Flow"    value={ahu?.outdoor_air_flow}  unit=" CFM"                 />
                    <StatRow label="Damper Position" value={ahu?.oa_damper_position} unit="%"                   />
                </Panel>
            </div>
    )
}
